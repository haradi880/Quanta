"""Lifecycle owner for the bundled standalone Garnet RESP process."""
...
class LocalRedisManager:
    def __init__(
        ...
        *,
        ...
        self.binary_path = (
            binary_path
            or os.environ.get("HARADIBOTS_GARNET_BIN")
            or os.environ.get("HARADIBOTS_REDIS_BIN")
        )
        self.host = host
        self.port = port
    ...
    def _resolve_binary(self) -> str:
        candidates = [
            self.binary_path,
            str(Path(sys.executable).resolve().parent / "GarnetServer.exe"),
            shutil.which("GarnetServer"),
        ]
        for candidate in candidates:
            if candidate and Path(candidate).is_file():
                return str(Path(candidate).resolve())
        raise FileNotFoundError(
            "bundled Garnet binary was not found; set HARADIBOTS_GARNET_BIN"
        )
    ...
    async def start(self, timeout_seconds: float = 5.0) -> str:
        ...
        while asyncio.get_running_loop().time() < deadline:
            if self.process.returncode is not None:
                raise RuntimeError(
                    f"bundled Garnet exited with code {self.process.returncode}"
                )
            if await self._reachable():
                os.environ["REDIS_URL"] = f"redis://{self.host}:{self.port}/0"
                return os.environ["REDIS_URL"]
        ...
        await self.stop()
        raise TimeoutError("bundled Garnet did not become ready")
